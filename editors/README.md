...
The Zed extension lives in [zed](zed). It starts `dyno lsp` through Zed's
extension API and includes Tune language metadata/query files. Zed also requires
a Tree-sitter grammar, so the split-ready grammar seed lives in
[tree-sitter-tune](tree-sitter-tune) until it is moved into its public
repository.
